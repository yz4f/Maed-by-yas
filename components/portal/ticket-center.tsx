...
export function TicketCenter({ lang, isDark, isStaff, onNotify }: TicketCenterProps) {
  ...
  const copyToClipboard = async (value: string, label: string) => {
    try {
      ...
      notify(isRtl ? `تم نسخ ${label}.` : `${label} copied.`);
    } catch {
      notify(isRtl ? 'تعذر النسخ من المتصفح.' : 'The browser could not copy this value.', 'error');
    }
  };
  ...
  const deleteSelectedTicket = async () => {
    if (!detail || deletingTicket) return;
    const ticket = detail.ticket;
    setDeletingTicket(true);
    try {
      const response = await fetch(`/api/tickets/${encodeURIComponent(ticket.id)}`, { method: 'DELETE' });
      const data = await response.json();
      if (!response.ok || !data.success) throw new Error(data.error || 'تعذر حذف التذكرة.');
      setTickets(current => current.filter(item => item.id !== ticket.id));
      setDetail(null);
      setMobileDetailOpen(false);
      setConfirmClose(false);
      notify(isRtl ? `تم حذف التذكرة ${ticket.number} نهائيًا.` : `${ticket.number} was deleted permanently.`);
      loadStats();
    } catch (error: any) {
      notify(error.message || (isRtl ? 'تعذر حذف التذكرة.' : 'Could not delete the ticket.'), 'error');
    } finally {
  ...
  return (
    <section className="ticket-center--graphite space-y-3 sm:space-y-4" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="support-command-shell grid gap-4 2xl:grid-cols-[220px_minmax(0,1fr)]">
        <aside className={`support-command-nav h-fit rounded-2xl border p-3 2xl:sticky 2xl:top-5 ${card}`}>
          ...
          <nav className="space-y-1">
            {[['overview', LayoutDashboard, isRtl ? 'نظرة عامة' : 'Overview'], ['tickets', TicketIcon, isRtl ? 'كل التذاكر' : 'All tickets'], ['open', Inbox, isRtl ? 'المفتوحة' : 'Open'], ['in_progress', Activity, isRtl ? 'قيد المعالجة' : 'In progress'], ['awaiting_user', Clock3, isRtl ? 'بانتظار العميل' : 'Waiting'], ['customers', UsersRound, isRtl ? 'العملاء' : 'Customers'], ['team', UserCheck, isRtl ? 'فريق الدعم' : 'Support team'], ['activity', BarChart3, isRtl ? 'النشاط' : 'Activity']].map(([value, Icon, label]: any) => {
              const selected = workspaceView === value || (['open', 'in_progress', 'awaiting_user'].includes(value) && workspaceView === 'tickets' && filter === value);
              const count = value === 'open' ? stats?.open : value === 'in_progress' ? stats?.inProgress : value === 'awaiting_user' ? stats?.awaitingUser : undefined;
              return <button key={value} onClick={() => { if (['open', 'in_progress', 'awaiting_user'].includes(value)) { setWorkspaceView('tickets'); setFilter(value as any); } else { setWorkspaceView(value); if (value === 'tickets') setFilter('all'); } }} className={`support-command-nav__item flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-right text-[11px] font-black transition ${selected ? 'support-command-nav__item--active' : `${textMuted} hover:bg-white/[0.055] hover:${textMain}`}`}><Icon className="h-4 w-4" /><span className="min-w-0 flex-1 truncate">{label}</span>{typeof count === 'number' && <span className="rounded-md bg-white/[0.08] px-1.5 py-0.5 text-[9px] tabular-nums">{count}</span>}</button>;
            })}
          </nav>
      ...
      <header className={`ticket-workspace-header support-command-header flex flex-wrap items-center justify-between gap-3 rounded-2xl border px-4 py-3 sm:px-5 ${card}`}>
        ...
        <div className="flex items-center gap-2"><button onClick={() => { loadTickets(); loadStats(); loadAgents(); notify(isRtl ? 'تم تحديث مساحة الدعم.' : 'Support workspace refreshed.', 'info'); }} className={`inline-flex h-9 w-9 items-center justify-center rounded-xl border transition hover:rotate-180 ${isDark ? 'border-white/[0.10] text-slate-300 hover:bg-white/[0.06]' : 'border-slate-900/[0.10] text-slate-700 hover:bg-slate-100'}`} title={isRtl ? 'تحديث' : 'Refresh'} aria-label={isRtl ? 'تحديث' : 'Refresh'}><RefreshCw className="h-3.5 w-3.5" /></button><button onClick={() => setNewOpen(true)} className="ticket-primary inline-flex h-9 items-center justify-center gap-1.5 rounded-xl px-3.5 text-xs font-black transition"><Plus className="h-3.5 w-3.5" />{isRtl ? 'تذكرة جديدة' : 'New ticket'}</button></div>
      </header>
      ...
      {workspaceView === 'tickets' && <div className="ticket-workspace grid gap-3 xl:grid-cols-[minmax(280px,0.88fr)_minmax(0,1.9fr)]">
        <aside className={`ticket-list-pane min-h-[520px] rounded-xl border p-2 ${mobileDetailOpen ? 'max-md:hidden' : ''} ${card}`}>
          ...
          <div className="scrollbar-none mb-2 flex gap-1 overflow-x-auto px-1 pb-1">
            {[
              ['all', isRtl ? 'الكل' : 'All'], ['new', isRtl ? 'جديدة' : 'New'], ['open', isRtl ? 'مفتوحة' : 'Open'], ['unassigned', isRtl ? 'غير مستلمة' : 'Unassigned'], ...(isStaff ? [['mine', isRtl ? 'استلمتها' : 'Mine'], ['in_progress', isRtl ? 'قيد المعالجة' : 'In progress'], ['awaiting_user', isRtl ? 'بانتظار المستخدم' : 'Awaiting user'], ['resolved', isRtl ? 'تم الحل' : 'Resolved']] : []),
        ...
        <main className={`ticket-detail-pane min-h-[520px] overflow-hidden rounded-xl border ${mobileDetailOpen ? '' : 'max-md:hidden'} ${card}`}>
          {mobileDetailOpen && <button onClick={() => setMobileDetailOpen(false)} className={`m-2 inline-flex items-center gap-1 rounded-md border px-2 py-1 text-[10px] font-black md:hidden ${isDark ? 'border-white/[0.1] text-zinc-300' : 'border-slate-900/[0.1] text-slate-600'}`}><ArrowLeft className="h-3.5 w-3.5" />{isRtl ? 'التذاكر' : 'Tickets'}</button>}{detailLoading ? <TicketDetailSkeleton isDark={isDark} /> : !detail ? (
            <div className={`flex min-h-[520px] flex-col items-center justify-center px-6 text-center ${textMuted}`}><div className={`ticket-empty-icon mb-3 rounded-xl p-3 ${isDark ? 'bg-white/[0.05]' : 'bg-zinc-100'}`}><FileText className="h-6 w-6 text-sky-400" /></div><h3 className={`text-sm font-black ${textMain}`}>{isRtl ? 'اختر تذكرة لعرض ملفها' : 'Select a ticket file'}</h3><p className="mt-1 max-w-sm text-xs">{isRtl ? 'ستظهر المعلومات والإجراءات وسجل المتابعة هنا.' : 'Details, actions, and activity will appear here.'}</p><button onClick={() => setNewOpen(true)} className="ticket-primary mt-4 inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-black"><Plus className="h-3.5 w-3.5" />{isRtl ? 'تذكرة جديدة' : 'New ticket'}</button></div>
                      ) : <TicketDetailView detail={detail} lang={lang} isDark={isDark} isStaff={isStaff} agents={agents} composer={composer} setComposer={setComposer} composerRef={composerRef} composerFiles={composerFiles} setComposerFiles={setComposerFiles} internalNote={internalNote} setInternalNote={setInternalNote} sending={sending} sendMessage={sendMessage} patchTicket={patchTicket} confirmClose={confirmClose} setConfirmClose={setConfirmClose} deletingTicket={deletingTicket} deleteSelectedTicket={deleteSelectedTicket} customerAction={customerAction} setCustomerAction={setCustomerAction} copyToClipboard={copyToClipboard} statusOptions={statusOptions} priorityOptions={priorityOptions} field={field} textMain={textMain} textMuted={textMuted} insertComposerToken={insertComposerToken} />}
        </main>
      </div>
...
function TicketListItem({ ticket, active, lang, isDark, onClick }: { ticket: SupportTicket; active: boolean; lang: 'ar' | 'en'; isDark: boolean; onClick: () => void }) {
  ...
  const isRtl = lang === 'ar';
  return <button onClick={onClick} className={`ticket-row ticket-conversation-row w-full text-start transition ${active ? 'ticket-list--active' : isDark ? 'hover:bg-white/[0.045]' : 'hover:bg-slate-900/[0.03]'}`} dir={isRtl ? 'rtl' : 'ltr'}>
    <div className="flex items-center gap-3 px-3 py-3">
      <div className="relative shrink-0"><Avatar image={ticket.userImage} name={ticket.userName} isDark={isDark} /><span className={`absolute -bottom-0.5 -left-0.5 h-2.5 w-2.5 rounded-full border-2 ${isDark ? 'border-[#111113]' : 'border-white'} ${status.dot}`} /></div>
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2"><span className={`truncate text-[11px] font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>{ticket.userName}</span><span className={`shrink-0 text-[9px] ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>{relativeTime(ticket.updatedAt, lang)}</span></div>
        <div className={`mt-1 line-clamp-1 text-[12px] font-black ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{ticket.title}</div>
        <div className={`mt-1.5 flex min-w-0 items-center gap-1.5 text-[9px] ${isDark ? 'text-slate-500' : 'text-slate-500'}`}><span className={`shrink-0 rounded-md border px-1.5 py-0.5 font-black ${status.color}`}>{isRtl ? status.ar : status.en}</span><span className="truncate">{ticket.number}</span><span className="opacity-40">•</span><span className={`shrink-0 rounded-md border px-1.5 py-0.5 font-black ${priority.color}`}>{isRtl ? priority.ar : priority.en}</span></div>
      </div>
    </div>
  </button>;
}
...
function TicketDetailView(props: any) {
  const { detail, lang, isDark, isStaff, agents, composer, setComposer, composerRef, composerFiles, setComposerFiles, internalNote, setInternalNote, sending, sendMessage, patchTicket, confirmClose, setConfirmClose, deletingTicket, deleteSelectedTicket, customerAction, setCustomerAction, copyToClipboard, priorityOptions, field, textMain, textMuted } = props;
  ...
  const permittedStatusOptions: Record<TicketStatus, TicketStatus[]> = {
    new: ['new', 'open', 'in_progress', 'awaiting_staff'],
    open: ['open', 'in_progress', 'awaiting_staff', 'awaiting_user', 'resolved'],
    in_progress: ['in_progress', 'open', 'awaiting_user', 'awaiting_staff', 'resolved'],
    awaiting_user: ['awaiting_user', 'in_progress', 'resolved'],
    awaiting_staff: ['awaiting_staff', 'open', 'in_progress'],
  ...
  return <div className="flex min-h-[520px] flex-col">
    <header className={`border-b px-3 py-3 sm:px-4 ${isDark ? 'border-white/[0.08]' : 'border-slate-900/[0.08]'}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {isStaff && <div className="flex flex-wrap gap-2">{!ticket.assignedAgentId && !closed && <button onClick={() => patchTicket({ action: 'claim' }, isRtl ? 'تم استلام التذكرة.' : 'Ticket claimed.')} className="inline-flex h-9 items-center gap-1.5 rounded-xl bg-sky-400 px-3 text-[10px] font-black text-slate-950 transition hover:scale-[1.02]"><UserCheck className="h-3.5 w-3.5" />{isRtl ? 'استلام التذكرة' : 'Claim ticket'}</button>}{closed ? <span className={`inline-flex h-9 items-center gap-1.5 rounded-xl border px-3 text-[10px] font-black ${isDark ? 'border-slate-400/25 bg-slate-400/10 text-slate-300' : 'border-slate-500/20 bg-slate-100 text-slate-600'}`}><LockKeyhole className="h-3.5 w-3.5" />{isRtl ? 'سجل قديم مغلق' : 'Legacy closed record'}</span> : <button onClick={() => setConfirmClose(true)} className={`inline-flex h-9 items-center gap-1.5 rounded-xl border px-3 text-[10px] font-black transition hover:scale-[1.02] ${isDark ? 'border-rose-400/25 bg-rose-400/10 text-rose-200 hover:bg-rose-400/18' : 'border-rose-500/25 bg-rose-50 text-rose-700 hover:bg-rose-100'}`}><Trash2 className="h-3.5 w-3.5" />{isRtl ? 'حذف نهائي' : 'Delete permanently'}</button>}</div>}
      </div>
    </header>
    ...
    <div className="grid flex-1 xl:grid-cols-[minmax(0,1fr)_230px]">
      ...
      <aside className={`ticket-metadata border-t px-3 py-3 xl:border-r xl:border-t-0 ${isDark ? 'border-white/[0.08] bg-black/[0.10]' : 'border-slate-900/[0.08] bg-slate-900/[0.025]'}`}>
        {isStaff && <section className={`mb-3 overflow-hidden rounded-2xl border p-3 ${isDark ? 'border-sky-300/15 bg-sky-400/[0.045]' : 'border-sky-500/15 bg-sky-50/70'}`}>
          ...
          <div className="mt-3 grid grid-cols-2 gap-1.5"><button onClick={() => copyToClipboard(ticket.userId, isRtl ? 'معرف العميل' : 'Customer ID')} className={`inline-flex h-7 items-center justify-center gap-1 rounded-lg border text-[9px] font-black ${isDark ? 'border-white/[0.1] text-slate-300 hover:bg-white/[0.06]' : 'border-slate-900/[0.1] text-slate-700 hover:bg-slate-100'}`}><Copy className="h-3 w-3" />{isRtl ? 'نسخ المعرف' : 'Copy ID'}</button><button onClick={() => copyToClipboard(customer?.email || ticket.userName, isRtl ? 'اسم العميل' : 'Customer name')} className={`inline-flex h-7 items-center justify-center gap-1 rounded-lg border text-[9px] font-black ${isDark ? 'border-white/[0.1] text-slate-300 hover:bg-white/[0.06]' : 'border-slate-900/[0.1] text-slate-700 hover:bg-slate-100'}`}><Copy className="h-3 w-3" />{isRtl ? 'نسخ الاسم' : 'Copy name'}</button></div>
    ...
    {confirmClose && <ConfirmClose lang={lang} isDark={isDark} deleting={deletingTicket} onCancel={() => !deletingTicket && setConfirmClose(false)} onConfirm={deleteSelectedTicket} />}
...
function TicketUpdateItem({ message, lang, isDark, textMain, textMuted }: any) {
  ...
  const staffSide = isRtl ? 'mr-auto' : 'ml-auto';
  const customerSide = isRtl ? 'ml-auto' : 'mr-auto';
  return <article className={`ticket-message flex ${internal ? 'justify-center py-2' : `${staff ? staffSide : customerSide} py-2`}`} dir={isRtl ? 'rtl' : 'ltr'}>
    <div className={`max-w-[88%] rounded-2xl border px-3 py-2.5 shadow-sm sm:max-w-[76%] ${internal ? (isDark ? 'border-amber-400/20 bg-amber-400/[0.075] text-amber-100' : 'border-amber-500/20 bg-amber-50 text-amber-900') : staff ? (isDark ? 'border-sky-300/20 bg-sky-400/[0.10]' : 'border-sky-500/20 bg-sky-50') : (isDark ? 'border-white/[0.09] bg-white/[0.045]' : 'border-slate-900/[0.08] bg-slate-50')}`}>
      <div className="flex flex-wrap items-center gap-x-1.5 gap-y-0.5"><span className={`text-[10px] font-black ${textMain}`}>{message.authorName}</span><span className={`text-[8px] ${textMuted}`}>{internal ? (isRtl ? 'ملاحظة داخلية' : 'Internal note') : staff ? (isRtl ? 'فريق الدعم' : 'Support team') : (isRtl ? 'العميل' : 'Customer')}</span>{internal && <span className="rounded border border-amber-400/25 bg-amber-400/10 px-1 py-0.5 text-[8px] font-bold text-amber-300">{isRtl ? 'داخلي' : 'Internal'}</span>}</div>
      <p className={`mt-1.5 whitespace-pre-wrap text-[11px] leading-5 ${internal ? (isDark ? 'text-amber-100/85' : 'text-amber-900') : isDark ? 'text-slate-200' : 'text-slate-700'}`}>{message.body}</p>
      {message.attachments?.length > 0 && <div className="mt-2 grid gap-1.5 sm:grid-cols-2">{message.attachments.map((attachment: TicketAttachment) => <AttachmentCard key={attachment.id} attachment={attachment} isDark={isDark} />)}</div>}
      <div className={`mt-2 text-[8px] ${textMuted}`}>{relativeTime(message.createdAt, lang)} · {formatDate(message.createdAt, lang)}</div>
    </div>
  </article>;
}
...
function ConfirmClose({ lang, isDark, deleting, onCancel, onConfirm }: { lang: 'ar' | 'en'; isDark: boolean; deleting: boolean; onCancel: () => void; onConfirm: () => void }) { const ar = lang === 'ar'; return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/70 p-5 backdrop-blur-sm"><div className={`w-full max-w-md rounded-3xl border p-6 shadow-2xl ${isDark ? 'border-rose-400/20 bg-[#171015]' : 'border-rose-500/15 bg-white'}`} dir={ar ? 'rtl' : 'ltr'}><div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-rose-400/12 text-rose-300"><Trash2 className="h-6 w-6" /></div><h4 className={`text-lg font-black ${isDark ? 'text-white' : 'text-slate-950'}`}>{ar ? 'حذف التذكرة نهائيًا؟' : 'Delete this ticket permanently?'}</h4><p className={`mt-2 text-sm leading-6 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{ar ? 'سيتم حذف التذكرة وجميع الرسائل والملفات وسجل النشاط المرتبط بها فورًا. لن تبقى تذكرة مغلقة في القائمة، ولا يمكن استعادتها أو إعادة فتحها.' : 'The ticket, all messages, attachments, and activity history will be removed immediately. It will not remain in the queue and cannot be restored or reopened.'}</p><div className={`mt-4 rounded-xl border px-3 py-2 text-[11px] font-bold ${isDark ? 'border-rose-400/15 bg-rose-400/[0.06] text-rose-100/80' : 'border-rose-500/15 bg-rose-50 text-rose-800'}`}>{ar ? 'هذا إجراء نهائي وغير قابل للتراجع.' : 'This is a final, irreversible action.'}</div><div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button disabled={deleting} onClick={onCancel} className={`rounded-xl px-4 py-2.5 text-sm font-black disabled:opacity-50 ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{ar ? 'إلغاء' : 'Cancel'}</button><button disabled={deleting} onClick={onConfirm} className="inline-flex items-center justify-center gap-2 rounded-xl bg-rose-400 px-4 py-2.5 text-sm font-black text-slate-950 disabled:opacity-60">{deleting && <Loader2 className="h-4 w-4 animate-spin" />}{deleting ? (ar ? 'جارٍ الحذف...' : 'Deleting...') : (ar ? 'حذف نهائي' : 'Delete permanently')}</button></div></div></div>; }
