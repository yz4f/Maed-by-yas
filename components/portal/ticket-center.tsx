...
export function TicketCenter({ lang, isDark, isStaff, onNotify }: TicketCenterProps) {
  ...
  }, [filter, priorityFilter, departmentFilter, query]);
    ...
    <section className="ticket-center--graphite space-y-5" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(290px,0.85fr)_minmax(0,1.85fr)]">
        <aside className={`min-h-[620px] rounded-3xl border p-3 ${card}`}>
          ...
          <div className="space-y-2 px-1 pb-3"><div className={`flex items-center gap-2 rounded-xl border px-3 ${field}`}><Search className="h-4 w-4 opacity-50" /><input value={query} onChange={event => setQuery(event.target.value)} placeholder={isRtl ? 'بحث برقم أو عنوان...' : 'Search tickets...'} className="h-10 min-w-0 flex-1 bg-transparent text-xs outline-none" /></div><div className="grid grid-cols-2 gap-2"><select value={priorityFilter} onChange={event => setPriorityFilter(event.target.value as 'all' | TicketPriority)} className={`h-9 rounded-lg border px-2 text-[10px] font-bold outline-none ${field}`}><option value="all">{isRtl ? 'كل الأولويات' : 'All priorities'}</option>{(Object.keys(priorityMeta) as TicketPriority[]).map(value => <option key={value} value={value}>{isRtl ? priorityMeta[value].ar : priorityMeta[value].en}</option>)}</select><select value={departmentFilter} onChange={event => setDepartmentFilter(event.target.value as 'all' | TicketDepartment)} className={`h-9 rounded-lg border px-2 text-[10px] font-bold outline-none ${field}`}><option value="all">{isRtl ? 'كل الأقسام' : 'All departments'}</option>{(Object.keys(departmentLabel) as TicketDepartment[]).map(value => <option key={value} value={value}>{isRtl ? departmentLabel[value].ar : departmentLabel[value].en}</option>)}</select></div></div>
...
function TicketDetailView(props: any) {
  const { detail, lang, isDark, isStaff, agents, composer, setComposer, composerRef, composerFiles, setComposerFiles, internalNote, setInternalNote, sending, sendMessage, patchTicket, confirmClose, setConfirmClose, statusOptions, priorityOptions, field, textMain, textMuted, insertComposerToken } = props;
      ...
      <div className="min-w-0 p-4 sm:p-5">
        ...
        <div className={`mt-5 rounded-2xl border p-3 sm:p-4 ${isDark ? 'border-sky-100/[0.12] bg-slate-950/48' : 'border-slate-900/[0.10] bg-white/80'}`}>
          {closed ? <div className={`flex items-center gap-3 p-3 text-sm ${textMuted}`}><Archive className="h-5 w-5 text-slate-400" /><div><div className={`font-black ${textMain}`}>{isRtl ? 'تم حل هذه التذكرة وإغلاقها.' : 'This ticket has been resolved and closed.'}</div><button onClick={() => patchTicket({ action: 'update', status: 'open' }, isRtl ? 'تمت إعادة فتح التذكرة.' : 'Ticket reopened.')} className="mt-1 font-bold text-sky-400 hover:underline">{isRtl ? 'إعادة فتح التذكرة' : 'Reopen ticket'}</button></div></div> : <>
            ...
            <textarea ref={composerRef} value={composer} onChange={(event: React.ChangeEvent<HTMLTextAreaElement>) => setComposer(event.target.value)} onKeyDown={(event: React.KeyboardEvent<HTMLTextAreaElement>) => { if (event.key === 'Enter' && !event.shiftKey && (composer.trim() || composerFiles.length)) { event.preventDefault(); sendMessage(); } }} placeholder={internalNote ? (isRtl ? 'اكتب ملاحظة للفريق...' : 'Write a note for your team...') : (isRtl ? 'اكتب ردك هنا...' : 'Write your reply here...')} className={`min-h-[110px] w-full resize-y rounded-xl border p-3 text-sm outline-none transition focus:border-sky-400/55 ${field}`} />
