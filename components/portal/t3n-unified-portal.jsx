...
import { Shield, Key, Download, Copy, Check, RefreshCw, LogOut, ExternalLink, Users, Package, Activity, Layers, Sparkles, Plus, Search, UserCheck, FileText, HelpCircle, Clock, CheckCircle2, Lock, ArrowRight, X, Edit3, Eye, EyeOff, Save, Trash2, ShoppingCart, AlertCircle, Moon, Sun, LayoutDashboard, MessageSquare, User, Globe, AlertTriangle, Unlock } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
export function T3NUnifiedPortal({ initialProducts }) {
    ...
    const renderBrandText = (text) => {
        return (<span className="notranslate inline-flex" translate="no" style={{ unicodeBidi: 'isolate' }}>
        {text.split('').map((char, idx) => (<span key={idx} className="notranslate" translate="no" style={{ display: 'inline-block' }}>
            {char === ' ' ? '\u00A0' : char}
          </span>))}
      </span>);
    };
    ...
    // Guide Modal States
    const [guideModalProduct, setGuideModalProduct] = useState(null);
    const [guideView, setGuideView] = useState(null);
    // Admin Panel States
    const [adminStats, setAdminStats] = useState(null);
    const [bulkProductId, setBulkProductId] = useState(initialProducts[0]?.id || '');
    ...
    const [inventoryTab, setInventoryTab] = useState('codes');
    ...
    const [productSaveMessage, setProductSaveMessage] = useState(null);
    const [editingKeyId, setEditingKeyId] = useState(null);
    const [editingKeyText, setEditingKeyText] = useState('');
    const [keyActionMessage, setKeyActionMessage] = useState(null);
    ...
    const [adminSectionTab, setAdminSectionTab] = useState('products');
    ...
    const [selectedCustomerProducts, setSelectedCustomerProducts] = useState([]);
    const [selectedProductToGrant, setSelectedProductToGrant] = useState('');
    const [banReasonInput, setBanReasonInput] = useState('');
    const [banTypeInput, setBanTypeInput] = useState('permanent');
    ...
    const currentUser = React.useMemo(() => {
        ...
        if (session?.user) {
            return {
                id: session.user.id || session.user.discordId || 'user-discord-active',
                discordId: session.user.discordId || '1396965033316978839',
    ...
    useEffect(() => {
        if (activeTab === 'admin' && isAdmin) {
            loadDbProducts();
            if (adminSectionTab === 'overview')
                loadAdminStats();
            if (adminSectionTab === 'customers')
                loadAdminCustomersList();
            if (adminSectionTab === 'keys')
                loadAllKeysList();
            if (adminSectionTab === 'logs')
                loadAdminStats();
        }
    ...
    const handleRevokeUserProduct = async (userId, productId) => {
        if (!confirm('هل أنت متأكد من سحب هذا المنتج من حساب المشترك؟'))
            return;
        try {
    ...
    const handleGrantProduct = async (userId) => {
    ...
    const handleDeleteUserAccount = async (userId, userName) => {
        if (!confirm(`هل أنت متأكد من حذف حساب العميل ${userName} نهائياً؟`))
            return;
        try {
    ...
    // Redeem Key Handler
    const handleRedeemKey = async (e, keyToRedeem) => {
        if (e)
            e.preventDefault();
    ...
    const openInventoryModal = async (product, defaultTab = 'codes', openBulk = false) => {
    ...
    const loadInventoryKeys = async (productId) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (inventoryProduct) {
                    ...
                    if (idx !== -1)
                        initialProducts[idx].stockKeysCount = (data.keys || []).length;
                }
            }
        }
        catch (e) {
    ...
    const handleRevokeAndBan = async (userId, keyId) => {
        if (!userId)
            return;
        if (!confirm('هل أنت متأكد من حظر المستخدم وإلغاء مفتاحه وسحب المنتج منه؟ لا يمكن التراجع عن هذا الإجراء.'))
            return;
        try {
    ...
    const handleDeleteAllKeys = async () => {
        if (!inventoryProduct)
            return;
        if (!confirm(`هل أنت تأكد من حذف جميع الأكواد المتاحة (${inventoryKeys.length} كود) لهذا المنتج؟`))
            return;
        try {
            const res = await fetch('/api/keys', {
                method: 'DELETE',
    ...
    const handleStartEditKey = (keyItem) => {
        setEditingKeyId(keyItem.id);
        setEditingKeyText(keyItem.key);
    };
    const handleSaveKeyEdit = async (keyId) => {
        if (!editingKeyText.trim())
            return;
        try {
            const res = await fetch('/api/keys', {
                method: 'PUT',
    ...
    const handleDeleteProductPermanently = async () => {
        if (!inventoryProduct)
            return;
        if (!confirm(`هل أنت تأكد من حذف المنتج "${inventoryProduct.name}" نهائياً من المتجر وكافة العملاء؟`))
            return;
        try {
    ...
    const handleBulkAddKeys = async () => {
        if (!bulkKeysText.trim() || !inventoryProduct)
            return;
        ...
        const optimisticKeys = rawKeys.map((k, i) => ({
            id: `temp-${Date.now()}-${i}`,
            key: k,
    ...
    const handleAddSingleKey = async () => {
        if (!singleKeyText.trim() || !inventoryProduct)
            return;
        try {
            const res = await fetch('/api/admin/keys/stock', {
                method: 'POST',
    ...
    if (!isLoggedIn) {
        return (<div className={`min-h-screen ${isDark ? 'bg-[#030303] text-[#F4F4F5]' : 'bg-[#FAFAFA] text-[#09090B]'} flex flex-col items-center justify-center p-4 relative overflow-hidden select-none transition-colors duration-500`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        ...
        <div className="fixed top-5 z-50 flex items-center gap-3 transition-all" style={{ [lang === 'ar' ? 'left' : 'right']: '1.25rem' }}>
          {/* Theme Switcher */}
          <button onClick={() => setTheme(isDark ? 'light' : 'dark')} className={`p-2.5 rounded-full ${isDark ? 'bg-[#09090B]/85 border-white/[0.06] hover:border-white/20 text-white' : 'bg-white/85 border-slate-200 hover:border-slate-350 text-slate-800'} border backdrop-blur-md shadow-2xl flex items-center justify-center transition-all hover:scale-105 cursor-pointer`} title={isDark ? 'Switch to Light Mode' : 'Switch to Dark Mode'}>
            {isDark ? <Sun className="w-4 h-4"/> : <Moon className="w-4 h-4"/>}
          </button>

          {/* Language Switcher */}
          <button onClick={() => setLang(lang === 'ar' ? 'en' : 'ar')} className={`px-4 py-2 rounded-full ${isDark ? 'bg-[#09090B]/85 border-white/[0.06] hover:border-white/20 text-white' : 'bg-white/85 border-slate-200 hover:border-slate-350 text-slate-800'} border backdrop-blur-md shadow-2xl flex items-center justify-center gap-1.5 text-xs font-semibold tracking-wide transition-all hover:scale-105 cursor-pointer`} title={lang === 'ar' ? 'Switch to English' : 'التحويل إلى العربية'}>
        ...
        {/* Animated Login Card */}
        <AnimatePresence>
          <motion.div initial={{ opacity: 0, y: 30, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }} className={`w-full max-w-[400px] ${isDark ? 'bg-[#090A0E]/85 border-indigo-500/10 shadow-[0_0_50px_rgba(99,102,241,0.15)] shadow-[0_24px_80px_rgba(0,0,0,0.9)]' : 'bg-white/95 border-slate-250 shadow-[0_24px_80px_rgba(0,0,0,0.06)]'} border rounded-[32px] p-10 relative z-10 text-center space-y-8 backdrop-blur-2xl`}>
            {/* Logo Section */}
            <motion.div initial={{ scale: 0, rotate: -10 }} animate={{ scale: 1, rotate: 0 }} transition={{ type: "spring", stiffness: 180, damping: 18, delay: 0.15 }} className="flex justify-center">
            ...
            {/* Bottom Footer Link */}
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.45 }} className={`pt-6 border-t ${styles.borderSubtle} text-xs flex items-center justify-center gap-2`}>
              <span className="text-[#71717A] font-medium">{lang === 'ar' ? 'عضو جديد؟' : 'New member?'}</span>
              <a href="https://t3n-store-production.up.railway.app/" target="_blank" rel="noopener noreferrer" className={`font-bold ${isDark ? 'text-white hover:text-indigo-400' : 'text-indigo-600 hover:text-indigo-500'} transition-colors cursor-pointer`}>
        ...
        {/* Copyright Footer */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className={`absolute bottom-6 left-8 text-xs font-medium ${isDark ? 'text-slate-500' : 'text-slate-400'} tracking-wide`} dir="rtl">
          © 2026 جميع الحقوق محفوظة لمنصة {renderBrandText('تعن')}
        </motion.div>
 
        {/* Guest Key Redemption Modal */}
        {guestModalOpen && (<div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
            <div className="glass-card border border-brand-border rounded-2xl p-6 max-w-md w-full relative shadow-2xl">
              <button onClick={() => setGuestModalOpen(false)} className="absolute top-4 left-4 text-slate-400 hover:text-white p-1">
                <X className="w-5 h-5"/>
              </button>
              ...
              <form onSubmit={handleRedeemKey} className="space-y-4">
                <div>
                  ...
                  <input type="text" value={keyInput} onChange={(e) => setKeyInput(e.target.value)} placeholder="T3N-FORT-99999-PERM" className="w-full bg-brand-sidebar border border-brand-border rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-primary transition-colors font-mono tracking-wider"/>
                </div>

                {redeemMessage && (<div className={`p-3 rounded-xl text-xs font-medium ${redeemMessage.type === 'success' ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400' : 'bg-rose-500/10 border border-rose-500/20 text-rose-400'}`}>
                    {redeemMessage.text}
                  </div>)}

                <button type="submit" disabled={isRedeeming} className="w-full py-3 bg-gradient-to-r from-sky-500 to-indigo-600 hover:from-sky-400 hover:to-indigo-500 text-white font-semibold text-sm rounded-xl transition-all shadow-lg shadow-sky-500/20 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50">
    ...
    if (currentUser?.isBanned) {
        return (<div className={`min-h-screen ${isDark ? 'bg-[#030303] text-[#F4F4F5]' : 'bg-[#FAFAFA] text-[#09090B]'} flex flex-col items-center justify-center p-4 relative overflow-hidden select-none`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className={`w-full max-w-md ${isDark ? 'bg-[#0A0707]/90 border-red-500/25 shadow-[0_0_50px_rgba(239,68,68,0.15)] backdrop-blur-xl' : 'bg-red-55/90 border-red-200 shadow-xl'} border rounded-[32px] p-8 text-center space-y-6 relative z-10`}>
          ...
          <div className={`p-5 rounded-2xl ${isDark ? 'bg-black/50 border-white/5' : 'bg-white border-slate-200'} border text-right space-y-3.5`}>
            ...
            {currentUser.banType === 'temporary' && currentUser.banExpiresAt && (<div>
          ...
          <button onClick={handleLogout} className="w-full py-3 bg-red-600 hover:bg-red-550 text-white font-extrabold text-xs rounded-xl shadow-lg shadow-red-500/10 transition-all flex items-center justify-center gap-2 cursor-pointer hover:-translate-y-0.5 active:translate-y-0">
    ...
    return (<div className={`flex h-screen overflow-hidden ${styles.bgApp} selection:bg-indigo-500/20 transition-colors duration-500`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <aside className={`hidden md:flex flex-col w-[260px] ${styles.bgSidebar} shrink-0 h-full relative z-20`} style={{ borderLeftWidth: lang === 'ar' ? '1px' : '0', borderRightWidth: lang === 'ar' ? '0' : '1px' }}>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-none px-5 py-2 space-y-6 relative z-10">
          ...
          <div>
            ...
            <div className="space-y-1">
              <button onClick={() => setActiveTab('overview')} className={`w-full flex items-center gap-3.5 px-3.5 py-2.5 rounded-[12px] transition-all duration-300 group relative ${activeTab === 'overview'
          ...
          <div>
            ...
            <div className="space-y-1">
              <button onClick={() => setActiveTab('my-products')} className={`w-full flex items-center gap-3.5 px-3.5 py-2.5 rounded-[12px] transition-all duration-300 group relative ${activeTab === 'my-products'
            ? styles.sidebarActive
            : styles.sidebarInactive}`}>
              ...
              <button onClick={() => setActiveTab('redeem')} className={`w-full flex items-center gap-3.5 px-3.5 py-2.5 rounded-[12px] transition-all duration-300 group relative ${activeTab === 'redeem'
            ? styles.sidebarActive
            : styles.sidebarInactive}`}>
          ...
          <div>
            ...
            <div className="space-y-1">
              <a href="https://discord.gg/t3n" target="_blank" rel="noopener noreferrer" className={`w-full flex items-center gap-3.5 px-3.5 py-2.5 rounded-[12px] bg-transparent border border-transparent ${styles.sidebarInactive} transition-all duration-300 group cursor-pointer relative`}>
          ...
          <div>
            ...
            <div className="space-y-1">
              <button onClick={() => setActiveTab('profile')} className={`w-full flex items-center gap-3.5 px-3.5 py-2.5 rounded-[12px] transition-all duration-300 group relative ${activeTab === 'profile'
            ? styles.sidebarActive
            : styles.sidebarInactive}`}>
              ...
              {isAdmin && (<button onClick={() => setActiveTab('admin')} className={`w-full flex items-center gap-3.5 px-3.5 py-2.5 rounded-[12px] transition-all duration-300 group relative ${activeTab === 'admin'
                ? styles.sidebarActive
                : styles.sidebarInactive}`}>
        ...
        <div className={`p-4 relative z-10 border-t ${styles.borderSubtle} mt-auto w-full flex flex-col gap-2`}>
          ...
          <div className={`${styles.bgInnerCard} rounded-xl p-2.5 flex items-center justify-between shadow-sm`}>
            ...
            <img src={currentUser.image || 'https://cdn.discordapp.com/embed/avatars/0.png'} alt="Avatar" className={`w-8 h-8 rounded-lg object-cover ${isDark ? 'grayscale opacity-90' : ''}`} onError={(e) => { e.currentTarget.src = 'https://cdn.discordapp.com/embed/avatars/0.png'; }}/>
          </div>

          {/* Logout Button */}
          <button onClick={handleLogout} className={`w-full h-9 rounded-xl bg-transparent border ${styles.btnSecondary} text-[11px] font-semibold transition-all flex items-center justify-center gap-2 cursor-pointer`}>
      ...
      <main className="flex-grow h-full overflow-y-auto p-6 md:p-10 scrollbar-none">
        {currentUser?.warningMessage && (<div className="mb-6 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-500 flex items-center justify-between gap-4 animate-in fade-in slide-in-from-top-4 duration-300">
        ...
        {/* TAB 1: OVERVIEW */}
        {activeTab === 'overview' && (<div className="space-y-6 max-w-6xl mx-auto">
            {/* Welcome Banner Card */}
            <div className={`${styles.bgCard} rounded-[16px] p-6 flex items-center gap-5 relative overflow-hidden transition-all duration-300 shadow-sm`}>
              <img src={currentUser.image || 'https://cdn.discordapp.com/embed/avatars/0.png'} alt={currentUser.name} className={`w-14 h-14 rounded-full object-cover ${isDark ? 'grayscale opacity-90' : ''}`} onError={(e) => { e.currentTarget.src = 'https://cdn.discordapp.com/embed/avatars/0.png'; }}/>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <div className={`${styles.bgCard} rounded-[16px] p-4 animate-slide-up h-fit`} style={{ animationDelay: '0.2s' }}>
                ...
                <div className="space-y-0 flex flex-col">
                  {/* Quick Action 1 */}
                  <button onClick={() => setActiveTab('my-products')} className={`w-full p-3 rounded-xl hover:bg-black/[0.02] dark:hover:bg-white/[0.02] flex items-center justify-between transition-all duration-300 group cursor-pointer border-b ${styles.borderSubtle} last:border-0`}>
                    <div className="flex items-center gap-3">
                  ...
                  {/* Quick Action 2 */}
                  <button onClick={() => setActiveTab('redeem')} className={`w-full p-3 rounded-xl hover:bg-black/[0.02] dark:hover:bg-white/[0.02] flex items-center justify-between transition-all duration-300 group cursor-pointer border-b ${styles.borderSubtle} last:border-0`}>
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-xl border ${styles.borderNormal} bg-transparent flex items-center justify-center shrink-0 ${styles.textMuted}`}>
                  ...
                  {/* Quick Action 3 */}
                  <button onClick={() => showToast(lang === 'ar' ? 'تمت مزامنة رتب ديسكورد!' : 'Discord roles synced!')} className={`w-full p-3 rounded-xl hover:bg-black/[0.02] dark:hover:bg-white/[0.02] flex items-center justify-between transition-all duration-300 group cursor-pointer border-b ${styles.borderSubtle} last:border-0`}>
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-xl border ${styles.borderNormal} bg-transparent flex items-center justify-center shrink-0 ${styles.textMuted}`}>
                  ...
                  {/* Quick Action 4 */}
                  <a href="https://discord.gg/t3n" target="_blank" rel="noopener noreferrer" className={`w-full p-3 rounded-xl hover:bg-black/[0.02] dark:hover:bg-white/[0.02] flex items-center justify-between transition-all duration-300 group cursor-pointer border-b ${styles.borderSubtle} last:border-0`}>
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-xl border ${styles.borderNormal} bg-transparent flex items-center justify-center shrink-0 ${styles.textMuted}`}>
                        <MessageSquare className="w-4 h-4"/>
                      </div>
        ...
        {activeTab === 'my-products' && (<div className="space-y-8 max-w-6xl mx-auto">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setActiveTab('redeem')} className={`h-[40px] px-4 rounded-[12px] bg-transparent border ${styles.borderNormal} hover:bg-black/5 dark:hover:bg-white/5 ${styles.textTitle} text-xs font-bold transition-all flex items-center gap-2 cursor-pointer ${lang === 'ar' ? 'flex-row-reverse' : ''}`}>
                <Key className="w-4 h-4"/>
            ...
            {/* Products Grid */}
            {isLoadingProducts ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {[1, 2, 3].map((n) => (<div key={n} className={`${styles.bgCard} rounded-[20px] overflow-hidden flex flex-col p-5 space-y-4 animate-pulse`}>
              ...
              </div>) : userProducts.length === 0 ? (<div className={`${styles.bgCard} rounded-[20px] p-12 text-center max-w-lg mx-auto shadow-sm`}>
                ...
                <button onClick={() => setActiveTab('redeem')} className="h-[42px] px-6 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs rounded-[12px] transition-all cursor-pointer shadow-lg shadow-indigo-500/20">
                ...
                {userProducts.map((up) => (<div key={up.id} className="glass-card flex flex-col group animate-slide-up relative">
                    {/* PRODUCT IMAGE */}
                    <div className={`relative h-40 w-full overflow-hidden rounded-t-[16px] bg-black/[0.02] dark:bg-[#05070B] border-b ${styles.borderSubtle}`}>
                      <img src={up.product?.image || '/logo.png?v=6'} alt={up.product?.name || 'Product'} className={`w-full h-full transition-transform duration-700 group-hover:scale-105 ${!up.product?.image || up.product?.image === '/logo.png?v=6' ? 'object-contain p-8 opacity-60' : 'object-cover'}`} onError={(e) => {
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      ...
                      <div className="mb-5 relative group/key">
                        ...
                        return revealedKeys[up.id]
                            ...
                            <div className="flex items-center gap-1 shrink-0">
                              <button onClick={() => toggleKeyReveal(up.id)} className={`h-7 w-7 rounded-lg flex items-center justify-center transition-all cursor-pointer ${styles.textMuted} hover:${styles.textTitle} bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10`} title={revealedKeys[up.id] ? (lang === 'ar' ? 'إخفاء' : 'Hide') : (lang === 'ar' ? 'إظهار' : 'Show')}>
                                {revealedKeys[up.id] ? <EyeOff className="w-3.5 h-3.5"/> : <Eye className="w-3.5 h-3.5"/>}
                              </button>
                              <button onClick={() => copyKeyToClipboard(up.keyString || 'KEY-ACTIVATED-OK', up.id)} className={`h-7 w-7 rounded-lg flex items-center justify-center transition-all cursor-pointer shrink-0 ${copiedKeyId === up.id ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : `bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 ${styles.textMuted} hover:${styles.textTitle} border border-transparent`}`} title="Copy Key">
                      ...
                      <div className="grid grid-cols-2 gap-2 mt-auto">
                        <button onClick={() => handleDownload(up.productId, up.product?.name || 'Product')} className="col-span-2 h-10 bg-indigo-600 hover:bg-indigo-500 dark:bg-primary dark:hover:bg-primary-hover text-white font-bold text-xs rounded-xl flex items-center justify-center gap-2 cursor-pointer transition-all hover:-translate-y-0.5 shadow-md shadow-indigo-500/10 dark:shadow-brand-glow">
                          <Download className="w-4 h-4"/>
                          <span>{lang === 'ar' ? 'تحميل البرنامج' : 'Download Loader'}</span>
                        </button>

                        <button className={`h-9 bg-transparent dark:bg-[#0D1420] border ${styles.borderNormal} hover:${styles.borderHover} ${styles.textTitle} font-bold text-[11px] rounded-xl flex items-center justify-center gap-1.5 cursor-pointer transition-all`}>
        ...
        {/* TAB 3: REDEEM KEY */}
        {activeTab === 'redeem' && (<div className="space-y-8 max-w-xl mx-auto py-8 animate-slide-up">
            ...
            <div className={`${styles.bgCard} rounded-[20px] p-8 space-y-6 relative overflow-hidden group shadow-sm`}>
              ...
              <form onSubmit={handleRedeemKey} className="space-y-5 relative z-10">
                <div>
                  ...
                  <input type="text" dir="ltr" value={keyInput} onChange={(e) => setKeyInput(e.target.value)} placeholder="KEY-T3N-ABCD-1234-EFGH" className={`w-full ${styles.bgInnerCard} border ${styles.borderNormal} focus:border-indigo-500/50 dark:focus:border-white/20 rounded-xl px-5 py-4 text-sm ${styles.textTitle} focus:outline-none focus:ring-1 focus:ring-indigo-500/20 dark:focus:ring-white/10 transition-all font-mono tracking-widest text-center shadow-inner`}/>
                </div>

                {redeemMessage && (<div className={`p-4 rounded-xl text-xs font-bold flex items-center justify-center gap-2 ${redeemMessage.type === 'success' ? 'bg-[#22C55E]/10 border border-[#22C55E]/20 text-[#22C55E]' : 'bg-red-500/10 border border-red-500/20 text-red-400'}`}>
                    {redeemMessage.type === 'success' ? <CheckCircle2 className="w-4 h-4"/> : <AlertCircle className="w-4 h-4"/>}
                    <span>{redeemMessage.text}</span>
                  </div>)}

                <button type="submit" disabled={isRedeeming} className="w-full py-4 bg-indigo-650 hover:bg-indigo-600 text-white font-bold text-sm rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 hover:-translate-y-0.5 active:translate-y-0 shadow-lg shadow-indigo-500/20">
              ...
              <div className={`pt-6 border-t ${styles.borderSubtle} space-y-4 relative z-10`}>
                <a href="https://t3n-store-production.up.railway.app" target="_blank" rel="noreferrer" className={`w-full ${styles.bgInnerCard} border ${styles.borderSubtle} hover:${styles.borderNormal} rounded-xl p-4 flex items-center justify-between transition-all group`}>
                ...
                <a href="https://discord.gg/t3n" target="_blank" rel="noreferrer" className={`w-full ${styles.bgInnerCard} border ${styles.borderSubtle} hover:${styles.borderNormal} rounded-xl p-4 flex items-center justify-between transition-all group mt-3`}>
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-lg ${isDark ? 'bg-white/5 border border-white/10' : 'bg-black/5 border border-black/10'} flex items-center justify-center ${styles.textTitle} group-hover:scale-110 transition-transform`}>
        ...
        {/* TAB 5: PROFILE */}
        {activeTab === 'profile' && (<div className="space-y-8 max-w-2xl mx-auto py-6 animate-slide-up">
            <div>
              <h1 className={`text-3xl font-extrabold ${styles.textTitle} tracking-tight`}>
            ...
            <div className={`${styles.bgCard} rounded-[20px] p-6 flex flex-col sm:flex-row items-center gap-6 shadow-sm`}>
              <div className="relative group">
                ...
                <img src={currentUser.image || 'https://cdn.discordapp.com/embed/avatars/0.png'} alt={currentUser.name} className={`w-20 h-20 rounded-full object-cover relative z-10 border-2 border-indigo-500/20 shadow-md ${isDark ? 'grayscale opacity-90' : ''}`} onError={(e) => { e.currentTarget.src = 'https://cdn.discordapp.com/embed/avatars/0.png'; }}/>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div className={`${styles.bgCard} rounded-[20px] p-6 shadow-sm flex flex-col justify-between`}>
                ...
                <div className="flex gap-2">
                  <button onClick={() => setTheme('light')} className={`flex-1 py-2.5 rounded-xl border text-xs font-bold transition-all cursor-pointer ${!isDark
                ...
                : `${styles.bgInnerCard} ${styles.borderNormal} ${styles.textMuted} hover:${styles.textTitle}`}`}>
                  ...
                  <button onClick={() => setTheme('dark')} className={`flex-1 py-2.5 rounded-xl border text-xs font-bold transition-all cursor-pointer ${isDark
                ? 'bg-indigo-600 border-indigo-600 text-white shadow-md shadow-indigo-500/15'
                : `${styles.bgInnerCard} ${styles.borderNormal} ${styles.textMuted} hover:${styles.textTitle}`}`}>
              ...
              <div className={`${styles.bgCard} rounded-[20px] p-6 shadow-sm flex flex-col justify-between`}>
                ...
                <div className="flex gap-2">
                  <button onClick={() => setLang('ar')} className={`flex-1 py-2.5 rounded-xl border text-xs font-bold transition-all cursor-pointer ${lang === 'ar'
                ? 'bg-indigo-600 border-indigo-600 text-white shadow-md shadow-indigo-500/15'
                : `${styles.bgInnerCard} ${styles.borderNormal} ${styles.textMuted} hover:${styles.textTitle}`}`}>
                    العربية
                  </button>
                  <button onClick={() => setLang('en')} className={`flex-1 py-2.5 rounded-xl border text-xs font-bold transition-all cursor-pointer ${lang === 'en'
                ? 'bg-indigo-600 border-indigo-600 text-white shadow-md shadow-indigo-500/15'
                : `${styles.bgInnerCard} ${styles.borderNormal} ${styles.textMuted} hover:${styles.textTitle}`}`}>
                    English
                  </button>
            ...
            <div className={`${styles.bgCard} rounded-[20px] p-6 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4 border-l-4 border-l-indigo-500`}>
              ...
              <a href="https://discord.gg/t3n" target="_blank" rel="noreferrer" className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl transition-all shadow-md shadow-indigo-500/10 cursor-pointer shrink-0">
        ...
        {activeTab === 'admin' && isAdmin && (<div className="space-y-6 max-w-6xl mx-auto">
            {/* Top Admin Header */}
            <div className={`${styles.bgCard} border ${styles.borderNormal} rounded-2xl p-5 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4`}>
              ...
              {adminStats && (<div className="flex items-center gap-3 text-xs">
            ...
            <div className={`flex flex-wrap items-center gap-1.5 p-1.5 bg-black/5 dark:bg-[#090b10] border ${styles.borderSubtle} rounded-2xl w-fit`}>
              {[
                ...
                return (<button key={tab.id} onClick={() => setAdminSectionTab(tab.id)} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${isActive
            ...
            {adminSectionTab === 'products' && (<div className="space-y-4">
                ...
                {products.length === 0 ? (<div className="flex flex-col items-center justify-center p-16 border border-dashed border-indigo-500/20 dark:border-primary/20 bg-black/40 backdrop-blur-md rounded-[24px] text-center space-y-6 animate-slide-up">
                    ...
                    <button onClick={openAddProductModal} className="py-3 px-6 bg-gradient-to-r from-indigo-500 to-purple-650 hover:from-indigo-650 hover:to-purple-700 text-white font-extrabold text-xs rounded-xl shadow-xl shadow-indigo-500/10 flex items-center gap-2.5 transition-all duration-300 hover:scale-105 active:scale-95 cursor-pointer">
                      <Plus className="w-4 h-4"/>
                      <span>{lang === 'ar' ? 'إضافة منتج جديد الآن' : 'Add First Product'}</span>
                    </button>
                  </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-slide-up">
                    {products.map((product) => (<div key={product.id} className="glass-card rounded-[24px] overflow-hidden hover:border-indigo-500/40 dark:hover:border-primary/40 transition-all duration-300 flex flex-col group">
                        ...
                        <div className={`relative h-48 w-full bg-black/[0.02] dark:bg-[#050505] overflow-hidden border-b ${styles.borderSubtle}`}>
                          <img src={product.image || '/logo.png?v=6'} alt={product.name} className={`w-full h-full transition-transform duration-700 group-hover:scale-110 ${!product.image || product.image === '/logo.png?v=6' ? 'object-contain p-8 opacity-60 group-hover:opacity-100' : 'object-cover'}`} onError={(e) => {
                            e.currentTarget.src = '/logo.png?v=6';
                        ...
                        <div className="p-6 flex-1 flex flex-col">
                          ...
                          <div className="grid grid-cols-2 gap-3 mt-auto">
                            <button onClick={() => openInventoryModal(product, 'codes', true)} className="py-3 px-4 bg-indigo-650 hover:bg-indigo-600 dark:bg-primary dark:hover:bg-primary-hover text-white dark:text-black font-bold text-xs rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer hover:-translate-y-0.5 shadow-md shadow-indigo-500/20">
                              <Key className="w-4 h-4"/>
                              <span>{lang === 'ar' ? 'إضافة مفاتيح' : 'Add Keys'}</span>
                            </button>

                            <button onClick={() => openInventoryModal(product, 'data', false)} className={`py-3 px-4 bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 border ${styles.borderNormal} rounded-xl text-xs font-bold ${styles.textTitle} transition-all flex items-center justify-center gap-2 cursor-pointer hover:-translate-y-0.5`}>
            ...
            {adminSectionTab === 'customers' && (<div className="space-y-4 animate-slide-up">
                <div className="glass-card rounded-[24px] p-6 md:p-8 space-y-6">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="relative w-full md:w-80">
                      ...
                      <input type="text" value={searchCustomerQuery} onChange={(e) => setSearchCustomerQuery(e.target.value)} placeholder={lang === 'ar' ? 'ابحث باسم العميل أو إيميله أو Discord ID...' : 'Search by name, email, or Discord ID...'} className={`w-full ${styles.bgInnerCard} border ${styles.borderNormal} focus:border-indigo-500/50 dark:focus:border-primary/50 rounded-xl ${lang === 'ar' ? 'pr-11 pl-4' : 'pl-11 pr-4'} py-3 text-xs ${styles.textTitle} placeholder:${styles.textLightMuted} focus:outline-none focus:ring-1 focus:ring-indigo-500/20 dark:focus:ring-primary/30 transition-all shadow-inner`}/>
                    </div>
                  </div>
                  ...
                  <div className="overflow-x-auto scrollbar-none">
                    ...
                    .map((customer) => (<tr key={customer.id} className={`${styles.textTitle} hover:bg-black/[0.02] dark:hover:bg-white/[0.03] transition-colors duration-150`}>
                              <td className="py-3.5 pr-2 font-bold flex items-center gap-2">
                                <img src={customer.image || 'https://cdn.discordapp.com/embed/avatars/0.png'} alt={customer.name} className={`w-8 h-8 rounded-full border ${styles.borderNormal} ${isDark ? 'grayscale' : ''}`} onError={(e) => { e.currentTarget.src = 'https://cdn.discordapp.com/embed/avatars/0.png'; }}/>
                                <div>
                              ...
                              <td className="py-3.5 space-y-1">
                                <div className="flex items-center gap-2">
                                  <button onClick={() => openCustomerModal(customer)} className="px-3 py-1.5 bg-indigo-500/10 dark:bg-primary/10 hover:bg-indigo-500/25 dark:hover:bg-primary/20 border border-indigo-500/30 dark:border-primary/30 rounded-lg text-[10px] font-black text-indigo-600 dark:text-primary transition-all cursor-pointer flex items-center gap-1.5 hover:scale-[1.02]" title={lang === 'ar' ? 'عرض تفاصيل العميل وإدارة اشتراكاته ومفاتيحه' : 'View customer profile and manage licenses'}>
                                    <Edit3 className="w-3.5 h-3.5"/>
                                    <span>{lang === 'ar' ? 'إدارة العميل' : 'Manage'}</span>
                                  </button>

                                  <button onClick={() => handleDeleteUserAccount(customer.id, customer.name)} className="p-1.5 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 text-rose-500 rounded-lg transition-colors cursor-pointer hover:scale-[1.02]" title={lang === 'ar' ? 'حذف العميل نهائياً' : 'Delete user permanently'}>
            ...
            {adminSectionTab === 'keys' && (<div className="space-y-4 animate-slide-up">
                <div className="glass-card rounded-[24px] p-6 md:p-8 space-y-6">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <h3 className={`text-lg font-black ${styles.textTitle} flex items-center gap-2`}>
                    ...
                    <div className="relative w-full md:w-80">
                      <Search className={`w-4 h-4 absolute ${lang === 'ar' ? 'right-4' : 'left-4'} top-3.5 ${styles.textMuted}`}/>
                      <input type="text" value={searchKeysQuery} onChange={(e) => setSearchKeysQuery(e.target.value)} placeholder={lang === 'ar' ? 'ابحث بكود المفتاح (e.g. T3N-FORT...)...' : 'Search key code...'} className={`w-full ${styles.bgInnerCard} border ${styles.borderNormal} focus:border-indigo-500/50 dark:focus:border-primary/50 rounded-xl ${lang === 'ar' ? 'pr-11 pl-4' : 'pl-11 pr-4'} py-3 text-xs ${styles.textTitle} placeholder:${styles.textLightMuted} focus:outline-none focus:ring-1 focus:ring-indigo-500/20 dark:focus:ring-primary/30 transition-all font-mono shadow-inner`}/>
                    </div>
                  </div>

                  {/* Status Filters */}
                  <div className="flex gap-2">
                    <button onClick={() => setKeyStatusFilter('all')} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${keyStatusFilter === 'all' ? 'bg-indigo-600 text-white dark:bg-primary dark:text-black font-extrabold shadow-sm' : `bg-black/5 dark:bg-white/5 border ${styles.borderNormal} ${styles.textMuted} hover:bg-black/10 dark:hover:bg-white/10`}`}>
                      {lang === 'ar' ? 'الكل' : 'All'} ({allKeysList.length})
                    </button>
                    <button onClick={() => setKeyStatusFilter('unused')} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${keyStatusFilter === 'unused' ? 'bg-indigo-600 text-white dark:bg-primary dark:text-black font-extrabold shadow-sm' : `bg-black/5 dark:bg-white/5 border ${styles.borderNormal} ${styles.textMuted} hover:bg-black/10 dark:hover:bg-white/10`}`}>
                      {lang === 'ar' ? 'متاح فقط' : 'Available only'} ({allKeysList.filter((k) => !k.isUsed).length})
                    </button>
                    <button onClick={() => setKeyStatusFilter('used')} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${keyStatusFilter === 'used' ? 'bg-indigo-600 text-white dark:bg-primary dark:text-black font-extrabold shadow-sm' : `bg-black/5 dark:bg-white/5 border ${styles.borderNormal} ${styles.textMuted} hover:bg-black/10 dark:hover:bg-white/10`}`}>
                  ...
                  <div className="overflow-x-auto scrollbar-none">
                    <table className="w-full text-right text-xs font-mono">
                      ...
                      <tbody className={`divide-y ${styles.borderSubtle}`}>
                        {allKeysList
                    .filter((k) => {
                    if (keyStatusFilter === 'unused' && k.isUsed)
                        return false;
                    if (keyStatusFilter === 'used' && !k.isUsed)
                        return false;
                    if (!searchKeysQuery)
                        return true;
                    return k.key.toLowerCase().includes(searchKeysQuery.toLowerCase());
                })
                    .map((keyObj) => (<tr key={keyObj.id} className={`${styles.textTitle} hover:bg-black/[0.02] dark:hover:bg-white/[0.03] transition-colors duration-150`}>
                              ...
                              <td className="py-3.5 font-sans">
                                {keyObj.isUsed ? (<span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded text-[10px] font-bold border border-slate-200 dark:border-transparent">
                                    {lang === 'ar' ? 'مستعمل' : 'Used'}
                                  </span>) : (<span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 rounded text-[10px] font-bold">
                                    {lang === 'ar' ? 'متاح' : 'Available'}
                                  </span>)}
                              </td>
                              <td className="py-3.5 font-sans text-indigo-600 dark:text-primary font-semibold text-xs">
                                {keyObj.isUsed ? (allCustomersList.find((u) => u.id === keyObj.usedByUserId)?.name ||
                        allCustomersList.find((u) => u.id === keyObj.usedByUserId)?.discordId ||
                        (lang === 'ar' ? 'مستخدم غير معروف' : 'Unknown User')) : (<span className={`${styles.textLightMuted} font-mono`}>-</span>)}
                              </td>
                              ...
                              <td className={`py-3.5 font-sans flex items-center ${lang === 'ar' ? 'justify-end pl-2' : 'justify-start pr-2'} gap-2`}>
                                {keyObj.isUsed && (<button onClick={() => handleRevokeAndBan(keyObj.usedByUserId, keyObj.id)} className="px-2.5 py-1 bg-rose-600 hover:bg-rose-500 text-white rounded text-[10px] font-bold transition-all cursor-pointer hover:scale-105" title={lang === 'ar' ? 'حظر المستخدم وإلغاء المفتاح' : 'Revoke key and ban user'}>
                                    {lang === 'ar' ? 'إلغاء وحظر' : 'Revoke & Ban'}
                                  </button>)}
                                <button onClick={() => handleDeleteKey(keyObj.id)} className="px-2.5 py-1 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 rounded text-[10px] font-bold text-rose-500 transition-all cursor-pointer hover:scale-105">
                                  {lang === 'ar' ? 'حذف' : 'Delete'}
                                </button>
                              </td>
                            </tr>))}
            ...
            {adminSectionTab === 'logs' && (<div className="glass-card rounded-[24px] p-6 md:p-8 space-y-6 animate-slide-up">
                <h3 className={`text-lg font-black ${styles.textTitle} flex items-center gap-2`}>
                ...
                <div className="overflow-x-auto scrollbar-none">
                  <table className="w-full text-right text-xs">
                    ...
                    return (<tr key={log.id} className={`${styles.textTitle} hover:bg-black/[0.02] dark:hover:bg-white/5 transition-colors duration-200`}>
                            ...
                            <td className="py-4">
                              <div className="flex items-center gap-2">
                                <img src={logAvatar} alt={log.userName || 'User'} className="w-7 h-7 rounded-full border border-white/5 object-cover" onError={(e) => { e.currentTarget.src = 'https://cdn.discordapp.com/embed/avatars/0.png'; }}/>
                                <div>
                                  ...
                                  {logUser?.discordId && (<div className="text-[9px] text-slate-500 font-mono">ID: {logUser.discordId}</div>)}
                                </div>
                              </div>
                            </td>
            ...
            {adminSectionTab === 'overview' && adminStats && (<div className="space-y-6">
                {/* Primary Stats Row */}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-slide-up" style={{ animationDelay: '0.1s' }}>
                  ...
                  <div className="glass-card rounded-[24px] p-6 space-y-6">
                    ...
                    <div className="space-y-3">
                      {adminLogs.slice(0, 4).map((log) => (<div key={log.id} className={`flex items-center gap-4 p-3 ${styles.bgInnerCard} rounded-xl border ${styles.borderSubtle} hover:bg-black/5 dark:hover:bg-white/10 transition-colors`}>
                      ...
                      {adminLogs.length === 0 && (<div className={`text-center text-sm ${styles.textMuted} py-6`}>{lang === 'ar' ? 'لا توجد أنشطة مسجلة مؤخراً' : 'No recent activities'}</div>)}
                    </div>
                  </div>
                  ...
                  <div className="glass-card rounded-[24px] p-6 space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      {adminStats.unusedKeys < 5 && (<div className="flex items-center justify-between p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl group hover:bg-amber-500/20 transition-colors">
                          <div className="flex items-center gap-4">
      ...
      {/* ====================================================================
           ...
           ==================================================================== */}
      {inventoryModalOpen && inventoryProduct && (<div className="fixed inset-0 z-50 bg-black/60 dark:bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className={`${styles.bgPanel} rounded-[28px] w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-300 border`} style={{ borderColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)' }}>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5 scrollbar-thin">
              ...
              {inventoryTab === 'data' && (<div className="space-y-5 animate-slide-up">
                  {productSaveMessage && (<div className="p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-xs rounded-xl font-bold flex items-center justify-between">
                  ...
                  <div>
                    ...
                    <input type="text" value={editProductData.name} onChange={(e) => setEditProductData({ ...editProductData, name: e.target.value })} className={`w-full ${styles.bgInput} focus:border-indigo-500/50 rounded-xl px-4 py-3 text-sm focus:outline-none transition-all font-bold shadow-inner`}/>
                  </div>

                  <div>
                    <label className={`block text-xs font-bold ${styles.textMuted} mb-2`}>صورة المنتج (رابط مسار الصورة)</label>
                    <input type="text" value={editProductData.image} onChange={(e) => setEditProductData({ ...editProductData, image: e.target.value })} placeholder="/products/fortnite-unban.png" className={`w-full ${styles.bgInput} focus:border-indigo-500/50 rounded-xl px-4 py-3 text-xs font-mono focus:outline-none transition-all shadow-inner`}/>
                  </div>

                  <div>
                    <label className={`block text-xs font-bold ${styles.textMuted} mb-2`}>الوصف الشامل للمنتج</label>
                    <textarea rows={3} value={editProductData.description} onChange={(e) => setEditProductData({ ...editProductData, description: e.target.value })} className={`w-full ${styles.bgInput} focus:border-indigo-500/50 rounded-xl p-4 text-xs focus:outline-none transition-all leading-relaxed shadow-inner`}/>
                  </div>

                  <div className="grid grid-cols-2 gap-5">
                    <div>
                      <label className={`block text-xs font-bold ${styles.textMuted} mb-2`}>الإصدار (Version)</label>
                      <input type="text" value={editProductData.version} onChange={(e) => setEditProductData({ ...editProductData, version: e.target.value })} className={`w-full ${styles.bgInput} focus:border-indigo-500/50 rounded-xl px-4 py-3 text-xs font-mono focus:outline-none transition-all shadow-inner`}/>
                    </div>
                    <div>
                      <label className={`block text-xs font-bold ${styles.textMuted} mb-2`}>حجم الملف (File Size)</label>
                      <input type="text" value={editProductData.fileSize} onChange={(e) => setEditProductData({ ...editProductData, fileSize: e.target.value })} className={`w-full ${styles.bgInput} focus:border-indigo-500/50 rounded-xl px-4 py-3 text-xs focus:outline-none transition-all shadow-inner`}/>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-5">
                    <div>
                      ...
                      <input type="text" value={editProductData.category} onChange={(e) => setEditProductData({ ...editProductData, category: e.target.value })} className={`w-full ${styles.bgInput} focus:border-indigo-500/50 rounded-xl px-4 py-3 text-xs focus:outline-none transition-all shadow-inner`}/>
                    </div>
                    <div>
                      <label className={`block text-xs font-bold ${styles.textMuted} mb-2`}>عدد التحميلات</label>
                      <input type="number" value={editProductData.downloadsCount} onChange={(e) => setEditProductData({ ...editProductData, downloadsCount: Number(e.target.value) })} className={`w-full ${styles.bgInput} focus:border-indigo-500/50 rounded-xl px-4 py-3 text-xs font-mono focus:outline-none transition-all shadow-inner`}/>
                    </div>
                  </div>

                  <div className={`pt-4 border-t ${styles.borderNormal}`}>
                    <button onClick={handleSaveProductChanges} disabled={isSavingProduct} className="w-full py-4 bg-indigo-650 hover:bg-indigo-600 dark:bg-primary dark:hover:bg-primary-hover text-white font-black text-xs rounded-xl shadow-lg shadow-indigo-500/10 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 hover:-translate-y-0.5">
              ...
              {inventoryTab === 'custom' && (<div className="space-y-5 animate-slide-up">
                  {productSaveMessage && (<div className="p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-xs rounded-xl font-bold flex items-center justify-between">
                      <span>{productSaveMessage}</span>
                      <Check className="w-4 h-4"/>
                    </div>)}
                  ...
                  <div>
                    ...
                    <input type="text" value={editProductData.videoUrl} onChange={(e) => setEditProductData({ ...editProductData, videoUrl: e.target.value })} className={`w-full ${styles.bgInput} focus:border-indigo-500/50 rounded-xl px-4 py-3 text-xs font-mono focus:outline-none transition-all shadow-inner`} placeholder="https://www.youtube.com/watch?v=..."/>
                  </div>

                  <div>
                    <label className={`block text-xs font-bold ${styles.textMuted} mb-2`}>رابط دليل الاستخدام (Discord / Docs)</label>
                    <input type="text" value={editProductData.guideUrl} onChange={(e) => setEditProductData({ ...editProductData, guideUrl: e.target.value })} className={`w-full ${styles.bgInput} focus:border-indigo-500/50 rounded-xl px-4 py-3 text-xs font-mono focus:outline-none transition-all shadow-inner`} placeholder="https://discord.gg/t3n"/>
                  </div>

                  <div>
                    <label className={`block text-xs font-bold ${styles.textMuted} mb-2`}>رابط تحميل الملف (File Download URL)</label>
                    <input type="text" value={editProductData.fileUrl} onChange={(e) => setEditProductData({ ...editProductData, fileUrl: e.target.value })} className={`w-full ${styles.bgInput} focus:border-indigo-500/50 rounded-xl px-4 py-3 text-xs font-mono focus:outline-none transition-all shadow-inner`} placeholder="/uploads/spoofer.exe"/>
                  </div>

                  <div>
                    <label className={`block text-xs font-bold ${styles.textMuted} mb-2`}>لون البطاقة (Theme Accent)</label>
                    <select value={editProductData.cardColor} onChange={(e) => setEditProductData({ ...editProductData, cardColor: e.target.value })} className={`w-full ${styles.bgInput} focus:border-indigo-500/40 rounded-xl px-4 py-3 text-xs focus:outline-none transition-all cursor-pointer font-bold shadow-inner`}>
                  ...
                  <div className={`pt-4 border-t ${styles.borderNormal} space-y-3`}>
                    ...
                    {inventoryProduct.id !== 'new' && (<button onClick={handleDeleteProductPermanently} className="w-full py-2.5 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 text-rose-500 dark:text-rose-450 font-extrabold text-xs rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer hover:scale-[1.01]">
              ...
              {inventoryTab === 'codes' && (<div className="space-y-5 animate-slide-up">
                  
                  {keyActionMessage && (<div className="p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-xs rounded-xl font-bold flex items-center justify-between shadow-sm">
                  ...
                  <div className={`bg-indigo-500/5 dark:bg-primary/5 border border-indigo-500/10 dark:border-primary/20 rounded-xl p-5 flex flex-col gap-3 relative overflow-hidden shadow-sm`}>
                    ...
                    {!bulkAddOpen && (<button onClick={() => setBulkAddOpen(true)} className="w-full py-3 bg-indigo-500/10 hover:bg-indigo-500/20 dark:bg-primary/10 dark:hover:bg-primary/20 border border-indigo-500/20 dark:border-primary/30 text-indigo-600 dark:text-primary text-xs font-bold rounded-xl transition-all cursor-pointer mt-2">
                  ...
                  {/* Bulk Add Panel */}
                  <AnimatePresence>
                    {bulkAddOpen && (<motion.div initial={{ opacity: 0, y: -10, height: 0 }} animate={{ opacity: 1, y: 0, height: 'auto' }} exit={{ opacity: 0, y: -10, height: 0 }} className={`${styles.bgCard} rounded-xl p-5 space-y-4 shadow-2xl relative overflow-hidden border border-indigo-500/10`}>
                        ...
                        <div className="flex justify-between items-center pr-2">
                          ...
                          {bulkKeysText.trim() && (<span className="text-xs font-bold bg-indigo-500/20 text-indigo-500 dark:text-primary px-3 py-1 rounded-lg">
                              {bulkKeysText.split(/[\n,]+/).map(k => k.trim()).filter(k => k.length > 0).length} مفاتيح
                            </span>)}
                        </div>
                        
                        <div className="relative">
                          <textarea rows={6} value={bulkKeysText} onChange={(e) => setBulkKeysText(e.target.value)} placeholder="الصق المفاتيح هنا...&#10;يمكنك الفصل بينها بمسافة أو فاصلة أو سطر جديد." className={`w-full ${styles.bgInput} rounded-xl p-4 text-sm font-mono placeholder:text-slate-550 focus:outline-none transition-all resize-y min-h-[120px] shadow-inner ${isAddingKeys ? 'border border-indigo-500 shadow-md text-indigo-500' : `border ${styles.borderNormal} focus:border-indigo-500/50 ${styles.textTitle}`}`} style={{ lineHeight: '1.8' }} dir="ltr"/>
                        </div>

                        {bulkMessage && (<div className="p-3 bg-indigo-500/10 text-indigo-500 dark:text-primary text-xs rounded-xl font-bold text-center border border-indigo-500/25 shadow-sm">
                            {bulkMessage}
                          </div>)}

                        <button onClick={handleBulkAddKeys} disabled={isAddingKeys || !bulkKeysText.trim()} className={`w-full py-4 font-bold text-sm rounded-xl transition-all cursor-pointer flex justify-center items-center gap-2 ${bulkKeysText.trim() ? 'bg-indigo-600 dark:bg-primary hover:bg-indigo-500 text-white shadow-lg shadow-indigo-500/15 hover:-translate-y-0.5' : 'bg-slate-100 dark:bg-white/5 text-slate-400 dark:text-slate-500 cursor-not-allowed border'}`}>
                          {isAddingKeys ? (<>
                  ...
                  <div className="pt-2">
                    ...
                    {isLoadingKeys ? (<div className={`flex flex-col items-center justify-center py-12 border ${styles.borderNormal} border-dashed rounded-xl ${isDark ? 'bg-black/20' : 'bg-slate-50'}`}>
                        ...
                        {inventoryKeys.map((keyItem) => (<div key={keyItem.id} className={`group ${isDark ? 'bg-black/40 hover:bg-white/5 border-white/10' : 'bg-slate-50 hover:bg-slate-100 border-slate-200'} border hover:border-indigo-500/30 rounded-xl overflow-hidden flex items-center justify-between p-4 gap-4 transition-all shadow-sm`}>
                            ...
                            <button onClick={() => handleDeleteKey(keyItem.id)} className="shrink-0 p-2.5 border border-transparent hover:border-red-500/30 hover:bg-red-500/10 text-slate-500 hover:text-red-500 rounded-lg transition-all cursor-pointer" title="حذف المفتاح">
                              <Trash2 className="w-4 h-4"/>
                            </button>
                          </div>))}

                        {inventoryKeys.length === 0 && !bulkAddOpen && !singleAddOpen && (<div className={`text-center py-10 border ${styles.borderNormal} border-dashed rounded-xl ${isDark ? 'bg-black/20' : 'bg-slate-50'}`}>
                        ...
                        <AnimatePresence>
                          {singleAddOpen && (<motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className={`border ${styles.borderNormal} rounded-xl p-3 flex flex-col sm:flex-row gap-3 shadow-lg ${isDark ? 'bg-black/40' : 'bg-slate-50'}`}>
                              <input type="text" value={singleKeyText} onChange={(e) => setSingleKeyText(e.target.value)} placeholder="أدخل المفتاح هنا..." className={`flex-1 ${styles.bgInput} rounded-lg px-4 py-3 text-sm font-mono placeholder:text-slate-500 focus:outline-none focus:border-indigo-500/50 text-left transition-all shadow-inner`} dir="ltr"/>
                              <div className="flex gap-2">
                                <button onClick={handleAddSingleKey} disabled={!singleKeyText.trim()} className="px-6 py-3 bg-indigo-650 hover:bg-indigo-600 dark:bg-primary dark:hover:bg-primary-hover text-white font-black text-xs rounded-lg transition-all cursor-pointer whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed shadow-md shadow-indigo-500/10">
                                  إضافة
                                </button>
                                <button onClick={() => setSingleAddOpen(false)} className={`px-5 py-3 ${isDark ? 'bg-white/5 hover:bg-red-500/20 border-white/10' : 'bg-slate-100 hover:bg-red-500/10 border-slate-200'} border text-slate-650 hover:text-red-500 font-bold text-xs rounded-lg transition-all cursor-pointer whitespace-nowrap`}>
                                  إلغاء
                                </button>
                              </div>
                            </motion.div>)}
                        ...
                        {/* Add New Key Button (+) */}
                        {!singleAddOpen && (<button onClick={() => setSingleAddOpen(true)} className={`w-full py-4 flex items-center justify-center gap-2 border border-dashed rounded-xl transition-all cursor-pointer mt-3 ${isDark ? 'bg-black/20 border-white/10 hover:border-primary/50 hover:bg-primary/5 text-slate-400 hover:text-primary' : 'bg-slate-50 border-slate-200 hover:border-indigo-500 hover:bg-indigo-50/50 text-slate-500 hover:text-indigo-600'}`}>
            ...
            <div className={`p-5 border-t ${styles.borderNormal} ${isDark ? 'bg-black/40' : 'bg-slate-50'} flex flex-col sm:flex-row items-center justify-between shrink-0 gap-3`}>
              <button onClick={() => setInventoryProduct(null)} className={`w-full sm:w-auto px-6 py-3 ${isDark ? 'bg-white/5 hover:bg-white/10 border-white/10' : 'bg-slate-100 hover:bg-slate-200 border-slate-250'} border ${styles.textTitle} font-bold text-sm rounded-xl transition-all cursor-pointer hover:-translate-y-0.5`}>
                إلغاء
              </button>
              <button onClick={handleSaveProductChanges} className="w-full sm:w-auto px-6 py-3 bg-indigo-650 hover:bg-indigo-600 dark:bg-primary dark:hover:bg-primary-hover text-white font-black text-sm rounded-xl shadow-lg shadow-indigo-500/15 transition-all cursor-pointer hover:-translate-y-0.5">
                حفظ التغييرات
              </button>
            </div>
          </div>
      ...
      {/* GUIDE MODAL */}
      {/* ------------------------------------------------------------------------------------------------ */}
      {guideModalProduct && guideView && (<div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative glass-card rounded-[24px] w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl animate-in fade-in zoom-in-95 duration-300">
            ...
            <div className="px-6 py-5 border-b border-white/10 flex items-start justify-between bg-black/40 relative overflow-hidden">
              ...
              <button onClick={() => { setGuideModalProduct(null); setGuideView(null); }} className="w-8 h-8 flex items-center justify-center rounded-lg bg-white/5 hover:bg-red-500/20 text-slate-400 hover:text-red-400 border border-white/10 transition-colors cursor-pointer shrink-0 z-10">
                <X className="w-4 h-4"/>
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto scrollbar-thin">
              {guideView === 'menu' && (<div className="grid grid-cols-1 sm:grid-cols-2 gap-6 animate-slide-up">
              ...
              {guideView === 'full' && (<div className="space-y-4 animate-slide-up">
                  ...
                  <div className="aspect-video w-full rounded-2xl border border-white/10 overflow-hidden bg-[#050505] flex flex-col items-center justify-center p-0 text-center relative shadow-2xl">
                    {guideModalProduct.product?.videoUrl ? (guideModalProduct.product.videoUrl.includes('youtube.com') || guideModalProduct.product.videoUrl.includes('youtu.be') ? (<iframe src={guideModalProduct.product.videoUrl
                        .replace('watch?v=', 'embed/')
                        .replace('youtu.be/', 'youtube.com/embed/')
                        .replace(/[?&]t=([0-9]+)s?/, (match, p1) => {
                        const prefix = match.startsWith('&') ? '&' : '?';
                        return `${prefix}start=${p1}`;
                    })} className="absolute inset-0 w-full h-full" allowFullScreen/>) : (<video src={guideModalProduct.product.videoUrl} className="absolute inset-0 w-full h-full object-contain bg-black" controls autoPlay/>)) : (<div className="p-8">
      ...
      {selectedAdminCustomer && (<div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in">
          <div className={`${styles.bgPanel} rounded-[28px] p-6 md:p-8 max-w-2xl w-full relative shadow-2xl max-h-[92vh] overflow-y-auto border scrollbar-none space-y-6`}>
            ...
            <div className="flex items-center gap-4.5 pb-5 border-b border-white/5">
              <img src={selectedAdminCustomer.image || 'https://cdn.discordapp.com/embed/avatars/0.png'} alt={selectedAdminCustomer.name} className="w-14 h-14 rounded-2xl border border-indigo-500/20 object-cover shadow-lg" onError={(e) => { e.currentTarget.src = 'https://cdn.discordapp.com/embed/avatars/0.png'; }}/>
              <div className="text-right">
                <h3 className={`text-[17px] font-black ${styles.textTitle} flex items-center gap-2`}>
                  <span>{selectedAdminCustomer.name}</span>
                  <span className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider ${selectedAdminCustomer.role === 'Boss'
                ? 'bg-purple-500/20 text-purple-400 border border-purple-500/30'
                : 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20'}`}>
                    {selectedAdminCustomer.role || 'Customer'}
                  </span>
                  {selectedAdminCustomer.isBanned && (<span className="px-2 py-0.5 rounded-md text-[9px] font-black bg-rose-500/20 text-rose-400 border border-rose-500/30">
            ...
            <div className="space-y-6 text-right">
              ...
              <div className="space-y-3">
                ...
                <div className={`${styles.bgInnerCard} rounded-2xl p-4 border`}>
                  {selectedCustomerProducts.length > 0 ? (<div className="space-y-2">
                      {selectedCustomerProducts.map((userProd) => {
                    const originalProd = products.find(p => p.id === userProd.productId);
                    return (<div key={userProd.id} className="flex items-center justify-between p-3 bg-black/10 dark:bg-white/[0.02] border border-white/5 rounded-xl">
                            <div>
                            ...
                            <button onClick={() => handleRevokeUserProduct(selectedAdminCustomer.id, userProd.productId)} className="px-3 py-1.5 bg-rose-500/10 hover:bg-rose-500/25 border border-rose-500/30 text-rose-500 text-[10px] font-black rounded-lg transition-all cursor-pointer">
                              {lang === 'ar' ? 'سحب وتعطيل' : 'Revoke Product'}
                            </button>
                          </div>);
                })}
                    </div>) : (<div className="text-xs text-slate-500 text-center py-4 font-medium">
              ...
              <div className="space-y-3">
                ...
                <div className={`${styles.bgInnerCard} rounded-2xl p-4 border flex flex-col sm:flex-row gap-3 items-end sm:items-center`}>
                  <div className="flex-1 w-full text-right">
                    ...
                    <select value={selectedProductToGrant} onChange={(e) => setSelectedProductToGrant(e.target.value)} className={`w-full h-11 px-3.5 rounded-xl ${styles.bgInput} text-xs font-bold focus:outline-none`}>
                  ...
                  <button onClick={() => handleGrantProduct(selectedAdminCustomer.id)} disabled={isProcessingAdminAction} className="h-11 px-5 bg-emerald-600 hover:bg-emerald-550 disabled:opacity-50 text-white text-xs font-black rounded-xl shadow-lg shadow-emerald-600/10 transition-all flex items-center justify-center gap-1.5 cursor-pointer w-full sm:w-auto">
              ...
              <div className="space-y-3">
                ...
                <div className={`${styles.bgInnerCard} rounded-2xl p-4 border space-y-4`}>
                  {selectedAdminCustomer.warningMessage && (<div className="p-3 bg-amber-500/10 border border-amber-500/20 text-amber-500 text-xs rounded-xl font-medium">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 items-end">
                    <div className="flex-grow w-full">
                      ...
                      <input type="text" value={warningMessageInput} onChange={(e) => setWarningMessageInput(e.target.value)} placeholder={lang === 'ar' ? 'مثال: الرجاء الالتزام بشروط الاستخدام...' : 'Enter warning...'} className={`w-full h-11 px-3.5 rounded-xl ${styles.bgInput} text-xs font-semibold focus:outline-none`}/>
                    </div>
                    <button onClick={() => handleWarnUser(selectedAdminCustomer.id)} disabled={isProcessingAdminAction} className="h-11 px-5 bg-amber-500 hover:bg-amber-450 disabled:opacity-50 text-white text-xs font-black rounded-xl shadow-lg shadow-amber-500/10 transition-all flex items-center justify-center gap-1.5 cursor-pointer w-full sm:w-auto">
                      <AlertTriangle className="w-4 h-4"/>
              ...
              <div className="space-y-3">
                ...
                <div className={`${styles.bgInnerCard} rounded-2xl p-4 border`}>
                  {selectedAdminCustomer.isBanned ? (<div className="space-y-4">
                      <div className="p-4 bg-rose-500/10 border border-rose-500/20 text-rose-400 text-xs rounded-xl space-y-2">
                        ...
                        {selectedAdminCustomer.banType === 'temporary' && selectedAdminCustomer.banExpiresAt && (<div>
                            <span className="font-bold">{lang === 'ar' ? 'ينتهي في:' : 'Expires:'}</span> {new Date(selectedAdminCustomer.banExpiresAt).toLocaleString('ar-SA')}
                          </div>)}
                      </div>
                      <button onClick={() => handleUnbanUser(selectedAdminCustomer.id)} disabled={isProcessingAdminAction} className="w-full py-3 bg-emerald-600 hover:bg-emerald-550 disabled:opacity-50 text-white text-xs font-black rounded-xl shadow-lg transition-all flex items-center justify-center gap-1.5 cursor-pointer">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="text-right">
                          ...
                          <input type="text" value={banReasonInput} onChange={(e) => setBanReasonInput(e.target.value)} placeholder={lang === 'ar' ? 'مخالفة شروط متجر تعن...' : 'Reason...'} className={`w-full h-11 px-3.5 rounded-xl ${styles.bgInput} text-xs font-semibold focus:outline-none`}/>
                        </div>
                        <div className="text-right">
                          <label className={`block text-[10px] font-bold ${styles.textLightMuted} mb-1.5`}>{lang === 'ar' ? 'نوع الحظر' : 'Ban Type'}</label>
                          <select value={banTypeInput} onChange={(e) => setBanTypeInput(e.target.value)} className={`w-full h-11 px-3.5 rounded-xl ${styles.bgInput} text-xs font-bold focus:outline-none`}>
                      ...
                      {banTypeInput === 'temporary' && (<div className="text-right animate-slide-up">
                          ...
                          <input type="datetime-local" value={banExpiresAtInput} onChange={(e) => setBanExpiresAtInput(e.target.value)} className={`w-full h-11 px-3.5 rounded-xl ${styles.bgInput} text-xs font-bold focus:outline-none`}/>
                        </div>)}

                      <button onClick={() => handleBanUser(selectedAdminCustomer.id)} disabled={isProcessingAdminAction} className="w-full py-3 bg-red-650 hover:bg-red-600 disabled:opacity-50 text-white text-xs font-black rounded-xl shadow-lg transition-all flex items-center justify-center gap-1.5 cursor-pointer">
              ...
              <div className="space-y-3">
                ...
                <div className={`${styles.bgInnerCard} rounded-2xl p-4 border max-h-48 overflow-y-auto scrollbar-none`}>
                  {allKeysList.filter(k => k.usedByUserId === selectedAdminCustomer.id).length > 0 ? (<div className="space-y-2">
                      {allKeysList.filter(k => k.usedByUserId === selectedAdminCustomer.id).map(keyObj => (<div key={keyObj.id} className="flex flex-col md:flex-row md:items-center justify-between gap-2 p-2.5 bg-black/10 dark:bg-white/[0.01] rounded-xl border border-white/5">
                          <div>
