...
export function SiteUpdatesAdmin({ lang, isDark, onNotify }: SiteUpdatesAdminProps) {
  ...
  const [pendingPublish, setPendingPublish] = useState<{ type: 'update'; update: SiteUpdate } | { type: 'status' } | null>(null);
  ...
  const publishProductStatus = async (confirmed = false) => {
    if (!confirmed) {
      setPendingPublish({ type: 'status' });
      return;
    }
