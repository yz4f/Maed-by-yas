...
import { ScreenAssistance } from '@/components/support/screen-assistance';
...
export function SupportSessionPage({ sessionId }: { sessionId: string }) {
  ...
  if (state === 'ready') {
    return <><AiChatModal open standalone sessionId={sessionId} onClose={() => router.push('/support')} lang="ar" isDark onNotify={() => undefined} onOpenGuide={(destination) => router.push(destination === 'issues' ? '/?tab=my-products&guide=issues' : '/?tab=my-products&guide=product')} /><ScreenAssistance sessionId={sessionId} /></>;
  }
